# CoreForge — round-by-round history

This is the archive behind `CLAUDE.md`. Every section is one shipped round: what
was asked, what was found, what shipped, which mutants escaped and why.

**Do not read this file.** Read the one section you need. `CLAUDE.md`'s
subsystem map names the round for each subsystem, and every section below is a
`## ` heading you can jump to:
...
The rules extracted from these rounds live in `CLAUDE.md` and are binding on
their own. Nothing here overrides that file; this is the evidence, not the law.
